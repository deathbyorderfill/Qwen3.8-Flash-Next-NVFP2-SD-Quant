...
def fused_moe_kernel_fp2(
    ...
    compute_type: tl.constexpr, filter_expert: tl.constexpr,
):
    """NVFP2/lm20 W2A16 grouped MoE GEMM (SDnvfp2). B: [E, N, K//4] uint8, 4 codes per
    ...
    KG: tl.constexpr = BLOCK_SIZE_K // 16
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        s4 = tl.reshape(tl.broadcast_to(s[:, None, :], (KG, 4, BLOCK_SIZE_N)), (KB, BLOCK_SIZE_N))
...
def invoke_fused_moe_kernel(
    ...
    if use_fp2_w2a16:
        ...
        assert block_shape is not None and block_shape[1] == 16
        # A.shape[1] is the true K; B.shape[2] is K//4 (packed)
        assert A.shape[1] % config["BLOCK_SIZE_K"] == 0 and config["BLOCK_SIZE_K"] % 16 == 0, (A.shape, config)
        ...
        fused_moe_kernel_fp2[grid](
            ...
            filter_expert=filter_expert, **config,
        )
        return
