...
def _trtllm_depth_ok(forward_batch) -> bool:
    """Depth rail for trtllm-gen sparse decode (corrupts >=~200k on SM121)."""
    import os

    limit = int(os.environ.get("SGLANG_QSA_TRTLLM_MAX_SEQ", "180000"))
    sl = getattr(forward_batch, "seq_lens_cpu", None)
    try:
        if sl is not None:
            m = int(sl.max()) if hasattr(sl, "max") else int(max(sl))
        else:
            m = int(forward_batch.seq_lens.max().item())
    except Exception:
        return False  # unknown depth -> exact path
    return m <= limit
...
def _resolve_trtllm_sparse_decode():
    ...
    if _os.environ.get("SGLANG_QSA_FORCE_TRTLLM") == "1":
        pass  # experiment: hasso5703/dgx-spark-qwen38 reports trtllm correct on GB10
    elif not (major == 10 or (major, minor) == (12, 0)):
        return None
    try:
...
class QwenSparseAttnBackend(AttentionBackend):
    ...
    def _forward_paged_attention(
        ...
        if trtllm_decode is not None and _trtllm_depth_ok(forward_batch):
