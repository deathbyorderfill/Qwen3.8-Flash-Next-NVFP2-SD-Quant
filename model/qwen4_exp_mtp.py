...
# SDnvfp2: wrap forward AFTER the class body (the class defines its own forward below the
# __init__, so an override inserted mid-class would be shadowed). Draft forwards flip the
# shared lm_head into slice mode; see qwen4_exp.py _DRAFT_SLICE.
_sdnvfp2_orig_forward = Qwen4ExpForCausalLMMTP.forward


def _sdnvfp2_sliced_forward(self, *args, **kwargs):
    import sglang.srt.models.qwen4_exp as _target_mod

    st = getattr(_target_mod, "_DRAFT_SLICE", None)
    import os as _os
    if _os.environ.get("SGLANG_DRAFT_SLICE_DEBUG") == "1" and (st is None or st.get("dbg_fwd", 0) < 5):
        import logging
        if st is not None:
            st["dbg_fwd"] = st.get("dbg_fwd", 0) + 1
        logging.getLogger(__name__).info(
            "[sdnvfp2 draft-slice dbg] draft forward: st=%s ids=%s lm_head id=%s qm=%s",
            st is not None, None if st is None else (st.get("ids") is not None), id(self.lm_head), type(self.lm_head.quant_method).__name__)
    if st is None or st.get("ids") is None:
        return _sdnvfp2_orig_forward(self, *args, **kwargs)
    st["active"] = True
    try:
        return _sdnvfp2_orig_forward(self, *args, **kwargs)
    finally:
...
Qwen4ExpForCausalLMMTP.forward = _sdnvfp2_sliced_forward
